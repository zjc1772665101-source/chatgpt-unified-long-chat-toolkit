...
- 回答大纲：优先使用 H1/H2；缺少标题时继续从 H3-H6、加粗段首和结构化段落生成降级大纲。
- 提示词库：创建、编辑、删除、置顶、搜索、分类、变量替换和 JSON 导入/导出。
...
提示词卡片只显示“置顶”和“编辑”。删除入口位于编辑弹窗中，避免误触。
...
发布前执行的验证包括 JavaScript 语法检查、原有模块保留检查、导航/提示词/导出静态检查，以及轻量运行时冒烟测试。当前版本通过 47 项检查。
