...
- Token 估算：可分别开关逐消息 Token 标签与本轮总览贴片；总览支持自动避让发送队列、位置预设与水平/垂直偏移，可在任意宽度强制使用 `输入/输出/上下文` 紧凑格式（如 `6/235/2.4k`），也可仅在窄屏自动紧凑；贴片文字、背景和边框颜色可独立覆盖，关闭覆盖后继续跟随 ChatGPT 明暗主题。复用会话导出的完整分支缓存，流式回答期间由当前 DOM 补齐最新输出。统计仅覆盖可见用户/助手文本，不包含系统指令、记忆、工具定义、隐藏推理等服务端上下文，因此只是近似值，不是官方 usage。
